...
class Stylesheet {
  ...
  function load_css_file($file, $origin = self::ORIG_AUTHOR) {
    ...
    if ( $this->_protocol == "" )
      ...
      $file = build_url_($this->_protocol, $this->_base_host, $this->_base_path, $filename);
...
//    }
  ...
  protected function _image($val) {
    ...
    if ( mb_strpos($val, "url") === false ) {
      ...
      if ( $parsed_url["protocol"] == "" && $this->get_protocol() == "" ) {
        ...
        $path = build_url_($this->get_protocol(),
                          $this->get_host(),
                          $this->get_base_path(),
                          $val);
      }
  ...
  private function _parse_import($url) {
    ...
    if ( $accept ) {
      ...
      // $url = build_url_($protocol == "php://" ? "file://" : $protocol, $host, $path, $url);
  ...
  private function _parse_font_face($str) {
    ...
    foreach($src[0] as $i => $value) {
      $source = array(
        ...
        "uri"    => $src[2][$i],
        "format" => $src[4][$i],
        "path"   => build_url_($this->_protocol, $this->_base_host, $this->_base_path, $src[2][$i]),
      );
